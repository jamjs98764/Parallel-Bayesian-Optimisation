...
class GPModel(BOModel):
    ...
    New changes
    """
    def predict_minimiser(self, X, with_noise=True):
        """
        Predictions with the model. Returns posterior means and standard deviations at X. 
        Note that this is different in GPy where the variances are given.
    ...
    def _gloabl_minimser(self,func):
        ntest = 2000;
        Xgrid = np.random.uniform(0.0, 1.0, (ntest, self.input_dim))
        Xtest = Xgrid
        Func_value_test = func(Xtest)
        idx_test = np.argmin(Func_value_test)
        X_start = Xtest[idx_test, :]
        # bnds = ((0.0, 1.0), (0.0, 1.0))
        self.lb = np.zeros(self.input_dim)
        self.ub = np.ones(self.input_dim)
    ...
    def get_minimiser(self):
        x_opt = self._gloabl_minimser(self.predict_minimiser)
        return x_opt
    """
    END
    """
...
class GPModel_MCMC(BOModel):
    ...
    def predict(self, X):
        """
        Predictions with the model for all the MCMC samples. Returns posterior means and standard deviations at X. 
        Note that this is different in GPy where the variances are given.
        """
    ...
    def predict_minimiser(self, X, with_noise=True):
        ...
        m, v = self.predict(X)
        # We can take the square root because v is just a diagonal matrix of variances
        marginalised_m = np.mean(m)
        return marginalised_m
