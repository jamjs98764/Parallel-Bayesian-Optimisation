...
def BO_test(test_func, BO_method, burnin=100, sample_size=50, interval=1):
    ...
    seed_size = 1 # Run BO experiment for x times 
    batch = False
    batch_size = 2
    heuristic = "kb"
    ...
    if batch == True:
        for j in range(seed_size):
            ...
            bayes_opt.initialise(x_ob, y_ob)
            X_optimum, Y_optimum = bayes_opt.iteration_step_batch(iterations=num_iterations, mc_burn=burnin, mc_samples=sample_size, \
                                                                              bo_method=BO_method, seed=seed, resample_interval=interval, \
