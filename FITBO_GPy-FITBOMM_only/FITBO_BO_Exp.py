...
def BO_test_v2(test_func, BO_method, burnin = 100, sample_size = 50, resample_interval = 1, \
    ...
    if batch == False: # Sequential
        ...
        for j in range(seed_size):
            ...
            original_results_IR = np.load(dir_name + 'A_results_L2,sequential.npy')
    ...
    if batch == True:
        ...
        for j in range(seed_size):
            ...
            # Loads old data and appends new one
            #####
            original_results_L2 = np.load(dir_name + 'A_results_L2,' + heuristic + '_heuristic')
            original_results_IR = np.load(dir_name + 'A_results_IR,' + heuristic + '_heuristic')
            new_results_L2 = np.vstack((original_results_L2, results_L2))
            new_results_IR = np.vstack((original_results_IR, results_IR))           
